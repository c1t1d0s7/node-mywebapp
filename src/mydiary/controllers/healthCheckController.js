...
function checkDatabaseConnection(req, res) {
  return sequelize
    .authenticate()
    .then(() => {
      console.log('Connection has been established successfully.');
      return sequelize.query(`SELECT 1 FROM ${config.database.name}.diaryentries LIMIT 1;`);
    })
    .then(() => {
      console.log('The "diaryentries" table exists.');
      res.status(200).send('OK');
    })
    .catch((error) => {
      console.error('Unable to connect to the database or "diaryentries" table does not exist:', error);
      res.status(503).send('Unable to connect to the database or "diaryentries" table does not exist.');
    });
}
